...
async function loadMetricRecords(userId: string, periodStart: Date, periodEnd: Date): Promise<MetricRecord[]> {
  const rows = await prisma.contentMetric.findMany({
    where: { userId, collectedAt: { gte: periodStart, lte: periodEnd } },
...
export async function calculateAnalyticsWorkflow(input: { userId: string; periodStart: Date; periodEnd: Date }) {
  const metrics = await loadMetricRecords(input.userId, input.periodStart, input.periodEnd);
...
export async function generateReviewReportWorkflow(input: { userId: string; periodStart: Date; periodEnd: Date }) {
