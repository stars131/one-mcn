...
import { defaultPlatform } from "@/lib/platforms/registry";
import { generateContentWorkflow, generateTopicsWorkflow } from "@/lib/workflows/ai-workflows";

export async function POST(request: Request, { params }: { params: { id: string } }) {
  try {
    const body = await readJson(request, z.object({ ipProfileId: z.string(), platform: z.string().default(defaultPlatform), contentType: z.string().default("图文") }));
