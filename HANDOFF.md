...
- 页面高度标准：AppShell 内的业务工作区不能直接用 `100vh` / `min-h-screen`，默认扣除导航、padding、标题和 gap，优先使用类似 `h-[calc(100vh-13rem)]` 的固定工作区高度，并把滚动交给内部区域
...
- 内容/日历：
  ...
  - 内容 Agent 左侧采用 ChatGPT 式结构：顶部只保留平台/内容类型，中间是对话滚动区，底部固定输入框
  - 内容 Agent 快捷选项集成在 Agent 回复气泡下方，作为回答后的可点击下一步
  - 内容 Agent 左侧工作区已按页面高度标准改为 `h-[calc(100vh-13rem)]`，避免外层页面为了看全核心对话而滚动
  - 内容 Agent 右侧预览区已升级为可编辑器，最近草稿支持“编辑”并调用 `PUT /api/contents/[id]` 保存修改
